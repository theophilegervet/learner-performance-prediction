...
if __name__ == "__main__":
    ...
    full_df = pd.read_csv(
        os.path.join("data", args_.setup, "preprocessed_data.csv"), sep="\t"
    )
    train_df = pd.read_csv(
        os.path.join("data", args_.setup, "preprocessed_data_train.csv"), sep="\t"
    )
    test_df = pd.read_csv(
        os.path.join("data", args_.setup, "preprocessed_data_test.csv"), sep="\t"
    )
    ...
    for setup_index in setup_page.index:
        ...
        if (
            args.exp_status == "DONE"
            or args[["result1", "result2", "result3"]].notnull().all()
        ):
        ...
        for rand_seed in range(int(args["repeat"])):
            ...
            train_data, val_data = get_data(
                train_df, int(args.max_length), randomize=True, stride=int(args.stride)
            )
            ...
            model = SAKT(
                ...
                num_skills,
                int(args.embed_size),
                int(args.num_attn_layers),
                int(args.num_heads),
                bool(args.encode_pos),
                int(args.max_pos),
                args.drop_prob,
            ).cuda()
            ...
            while True:  # Reduce batch size until it fits on GPU
                try:
                    # Train
                    param_str = "_".join(
                        [str(x) + str(y) for x, y in args.to_dict().items()]
                    )[:200]
                    optimizer = (
                        "adam" if "optimizer" not in args.index else args["optimizer"]
                    )
                    logger = Logger(os.path.join(args.logdir, param_str))
                    saver = Saver(
                        args.savedir,
                        param_str,
                        patience=10 if args_.setup != "ednet" else 3,
                    )
                    train(
                        train_data,
                        val_data,
            ...
            test_data, _ = get_data(
                test_df, int(args.max_length), train_split=1.0, randomize=False
            )
            test_batches = prepare_batches(
                test_data, int(args.batch_size), randomize=False
            )
            ...
            setup_page.loc[
                setup_index, "result{}".format(rand_seed + 1)
