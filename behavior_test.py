...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        for single_data in test_data[:10]:
            ...
            print(orig_output)
            perturb_input, pass_range = generate_test_case(orig_input, orig_output, perturb_flip_all, (1,), pass_increase)
            ...
            perturb_input, pass_range = generate_test_case(orig_input, orig_output, perturb_flip_all, (0,), pass_decrease)
            perturb_output = model(*(wrap_input(perturb_input)))
            perturb_output = torch.sigmoid(perturb_output)
            perturb_output = perturb_output[0][-1]
            print(perturb_output)
