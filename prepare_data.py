...
def prepare_assistments(
    ...
    bkt_split = np.random.randint(low=0, high=5, size=df["user_id"].nunique()).reshape(
        1, -1
    )
    ...
    train_df.to_csv(
        os.path.join(data_path, "preprocessed_data_train.csv"), sep="\t", index=False
    )
    test_df.to_csv(
        os.path.join(data_path, "preprocessed_data_test.csv"), sep="\t", index=False
    )
...
def prepare_squirrel_ai(min_interactions_per_user):
    ...
    test_df["timestamp"] = (test_df["timestamp"] - test_df["timestamp"].min()).astype(
        np.int64
    )
    ...
    sparse.save_npz(os.path.join(data_path, "q_mat.npz"), sparse.csr_matrix(Q_mat))
    train_df.to_csv(
        os.path.join(data_path, f"preprocessed_data_train.csv"), sep="\t", index=False
    )
    test_df.to_csv(
        os.path.join(data_path, f"preprocessed_data_test.csv"), sep="\t", index=False
    )
