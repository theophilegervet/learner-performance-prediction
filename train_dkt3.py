...
def train(train_data, val_data, model, optimizer, logger, saver, num_epochs, batch_size, lam):
    """Train DKT model.
    ...
    Arguments:
        ...
        batch_size (int)
        lam (float): hinge loss coefficient
    """
    ...
    for epoch in range(num_epochs):
        ...
        for item_inputs, KP_inputs, label_inputs, item_ids, KP_ids, labels in train_batches:
            ...
            metrics.store({'reg/train': reg.item()})
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train item hierarchical DKT.')
    parser.add_argument('--dataset', type=str)
    parser.add_argument('--logdir', type=str, default='runs/item_hierarchical_dkt')
    parser.add_argument('--savedir', type=str, default='save/item_hierarchical_dkt')
    ...
    Q_mat = torch.tensor(sparse.load_npz(os.path.join('data', args.dataset, 'q_mat.npz')).toarray())

    train_data, val_data = get_data(train_df, train_split=0.8)

    model = DKT3(int(full_df["item_id"].max()), int(full_df["skill_id"].max()), 
                 Q_mat, args.hid_size,args.embed_size, args.num_hid_layers, args.drop_prob).cuda()
    optimizer = Adam(model.parameters(), lr=args.lr)

    # Train
    param_str = f"{args.dataset},{args.lam}"
    logger = Logger(os.path.join(args.logdir, param_str))
    saver = Saver(args.savedir, param_str)
    train(train_data, val_data, model, optimizer, logger, saver, args.num_epochs,
          args.batch_size, args.lam)
    logger.close()
