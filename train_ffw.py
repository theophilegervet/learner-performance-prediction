...
def get_tensors(sparse):
    ...
    inputs = torch.tensor(dense[:, 4:], dtype=torch.float)
    item_ids = torch.tensor(dense[:, 1], dtype=torch.long)
    labels = torch.tensor(dense[:, 3], dtype=torch.float)
...
if __name__ == "__main__":
    ...
    model = FeedForward(input_size, args.hid_size, num_items, args.drop_prob).cuda()
    optimizer = Adam(model.parameters(), lr=args.lr)

    features_suffix = (args.X_file.split("-")[1]).split(".")[0]
    param_str = f'{args.dataset}, {features_suffix}'
